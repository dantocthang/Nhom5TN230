...
namespace Nhom5TN230.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            if (Session["role"] == null) return 0;//Tương đương người dùng
            else //Tương đương nhân viên đã đăng nhập
            {
                var role = Session["role"].ToString();
                if (role == "Admin") return 1; //Tương đương admin
                if (nhanvien == 0) return 2;//Đang đăng nhập với quyền nhân viên nhưng không cho nhân viên
            }
            return 1;
        }
